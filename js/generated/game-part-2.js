...
let boarMeat = 0, hareMeat = 0, deerMeat = 0, foxMeat = 0, birdMeat = 0, dragonFish = 0;
...
function dropGold(x, y, kind) {
  if(kind==='boar'||kind==='hare'||kind==='deer'||kind==='fox'||kind==='bird')return 0;
...
function grabGold() {
  let got = 0, meat = 0, hare = 0, deer = 0, fox = 0, bird = 0, kept = [];
  for (const g of loot) {
...
function drawLoot() {
  ...
  for (const g of loot) {
    ...
    if(g.kind==='boarMeat'||g.kind==='hareMeat'||g.kind==='deerMeat'||g.kind==='foxMeat'||g.kind==='birdMeat'){drawHuntingMeat(g.x,g.y+Math.round(Math.sin(g.t*3)*1.5));continue;}
...
function feedDragon(kind) {
  ...
  const fish = kind === "fish", hare = kind === "hare", deer = kind === "deer", fox = kind === "fox", bird = kind === "bird";
  const food=fish?"fish":hare?"hare meat":deer?"deer meat":fox?"fox meat":bird?"bird meat":"boar meat";
  if ((fish ? dragonFish : hare ? hareMeat : deer ? deerMeat : fox ? foxMeat : bird ? birdMeat : boarMeat) <= 0 && !devSafe) {
  ...
  if (fish) dragonFish--; else if(hare) hareMeat--; else if(deer) deerMeat--; else if(fox) foxMeat--; else if(bird) birdMeat--; else boarMeat--;
...
function interact() {
  ...
  if (!sayNpc && dragon.down && dragonHere() &&
    ...
    else if (birdMeat > 0) feedDragon("bird");
...
tap(document.getElementById('tEncounter'),()=>{
  arenaEncounter={hare:'boar',boar:'deer',deer:'fox',fox:'bird',bird:'combat',combat:'hare'}[arenaEncounter];refreshBuild();
});
  ...
  for (const f of features) {
    ...
    if (f.kind === "route")
      ...
      L.push("F arena " + f.id + " " + f.x + " " + f.y + " " +
             (f.r || 6) + " " + f.style + (['hare','boar','deer','fox','bird'].includes(f.encounter)?' '+f.encounter:''));
    else
