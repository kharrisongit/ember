...
function refillRing(a) {
  if (!a || ['hare','boar','deer','fox','bird'].includes(a.encounter) || a.templeRoom || cooling.has(ringKey(a)) || holy.has(ringKey(a))) return;
  let n = 0;
...
function arenaRim(a) {
  if(['hare','boar','deer','fox','bird'].includes(a.encounter))return [];
...
const HEALS = { potion: 0, elixir: 1, boarMeat: 2, hareMeat: 3, deerMeat: 4, foxMeat: 5, birdMeat: 6, dragonFish: 7 };
const USABLE = { potion: 1, elixir: 1, boarMeat: 1, hareMeat: 1, deerMeat: 1, foxMeat: 1, birdMeat: 1, dragonFish: 1, bomb: 1, dust: 1, bell: 1,
...
function doUse(it) {
  const act = it.key === "potion" ? drinkPotion
            ...
            : it.key === "birdMeat" ? () => feedDragon("bird")
...
function bagUse() {
  ...
  else if (it.key === "birdMeat") opts.push({ n: "FEED DRAGON", go: () => feedDragon("bird") });
