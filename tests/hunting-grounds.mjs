...
for(const species of ['hare','boar']){
const c=vm.createContext({console,TS:16,MAPID:'world',MD:{foes:[],features:[],felled:['130,60','900,500'],felled_rle:'60:131-133|500:901'},
 ...
 boarMeat:0,hareMeat:0,dragonFish:0,gold:50,graves:null,BOSS_KIND:/^never$/,WORTH:{},GOLD_DROP_MULTIPLIER:1.8,
...
if(species==='boar')run("features[2].encounter='boar';spawnHuntingAnimals()");
assert.equal(c.foes.length,3);assert(c.foes.every(f=>f.kind===species&&f.hp===(species==='hare'?2:3)));assert.equal(c.FOE_ART.hare,'hare');assert.equal(c.FOE_ART.boar,'br');
...
for(let i=0;i<(species==='hare'?2:3);i++){c.P.act={kind:'swing',t:4,hit:0};run('swingHits()');}
...
run("feedDragon('"+(species==='hare'?'hare':'meat')+"')");assert.equal(c[species+'Meat'],0);assert.equal(c.dragon.hp,5,'hunted meat uses dragon healing');
...
console.log('PASS: hare and boar hunts: exact route patch, species art, confinement, no walls, sword kills, meat collection, healing, timed respawn and additional hunting spots.');
