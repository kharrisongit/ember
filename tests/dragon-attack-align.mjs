...
for(const air of [false,true])for(const dir of ['n','ne','e','se','s','sw','w','nw'])for(const kind of ['slash','fire','ice','bolt','shadow']){
 ...
 assert.deepEqual(Array.from(api.offset(kind,dir,air)),[10,-4]);
}
...
elements.daResetAll.onclick();assert.equal(Object.keys(api.export().offsets.ground).length,0);elements.daClose.onclick();assert(!api.isOpen());
