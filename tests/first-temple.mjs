...
const clear=(m,x,y,gate=false)=>[[x-6,y-12],[x+6,y-12],[x-6,y-1],[x+6,y-1]].every(([px,py])=>inside(m,px,py)&&!m.roomBlocks.some(([l,t,r,b])=>px>=l&&px<r&&py>=t&&py<b)&&!(gate&&px>=288&&px<352&&py>=336&&py<352));
...
for(const [id,m] of Object.entries(W.maps)){
 ...
 assert(m.w>=40,`${id} has meaningful horizontal space`);assert(clear(m,...m.spawn));
 const reached=flood(m);
 for(const a of m.roomActors.filter(a=>a.houseLoot)){chests++;assert(reached.has([a.x,a.y+24].join(',')),id+' chest reachable');}
...
assert(!flood(sanctum,true).has('384,192'),'closed gate prevents reaching heartstone');
assert(flood(sanctum,false).has('384,192'),'open gate permits heartstone');
...
console.log('PASS: five expansive maps, all door approaches and arrivals, eight reachable chests, valid enemy spawns, guardian gate, single progression route, optional dead ends and untouched other temples.');
