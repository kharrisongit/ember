...
let finishFlight,flights=0,musicBegins=0;
c.window.EmberTitleScreen={depart:()=>{flights++;return new Promise(resolve=>{finishFlight=resolve;});}};
c.window.EmberTitleAudio={begin:()=>musicBegins++,fadeOut:async()=>{},fadeIn:async()=>{},finish(){}};
for(const start of ['act','bootNew','bootContinue','bootLoad']){
  ...
  const previousFlights=flights,previousMusic=musicBegins;
  dispatch(nodes.act,'touchstart');
  assert.equal(musicBegins,previousMusic+1,'First A starts music synchronously');
  assert.equal(run('BOOT.menuOpen'),false,'Menu waits until the rider is offscreen');
  assert.equal(run('BOOT.beginning'),true);assert.equal(nodes.bootBtns.style.display,'none');
  dispatch(nodes.act,'touchstart');key('a');dispatch(nodes.bootNew,'click');dispatch(nodes.bootContinue,'click');dispatch(nodes.bootLoad,'click');
  assert.equal(flights,previousFlights+1,'Repeated input cannot launch duplicate flights');
  assert.equal(run('gameplayStarted'),false,'Input during departure cannot start gameplay or load saves');
  finishFlight();for(let i=0;i<4;i++)await Promise.resolve();
  assert.equal(run('BOOT.menuOpen'),true);assert.equal(run('BOOT.beginning'),false);
  assert.equal(run('gameplayStarted'),false,'First A only opens the title menu');
...
assert.equal(hatchFrame(3,.6).y,landing);
