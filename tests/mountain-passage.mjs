...
assert.equal(saved.dragonIntroDone,true);assert.deepEqual(saved.dragonBanterSeen,['enemy:ent']);
assert.equal(saved.birdMeat,5);c.birdMeat=0;assert.equal(saved.foxMeat,4);c.foxMeat=0;assert.equal(saved.deerMeat,3);c.deerMeat=0;assert.equal(saved.hareMeat,2);c.hareMeat=0;assert.equal(saved.passageLayoutVersion,1);assert(saved.templeDefeated['passage3:0']);assert(Object.keys(saved.templeDefeated).some(k=>k.includes(':spikes:')));
for(const k of Object.keys(c.bossGone))delete c.bossGone[k];assert(run('loadGame(1)'));assert.deepEqual(c.bossGone,saved.templeDefeated);assert.equal(c.hareMeat,2,'hare meat survives reload');assert.equal(c.deerMeat,3,'deer meat survives reload');assert.equal(c.foxMeat,4,'fox meat survives reload');assert.equal(c.birdMeat,5,'bird meat survives reload');
