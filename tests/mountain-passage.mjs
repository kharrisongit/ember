...
Object.assign(c,{quest:1,smithUpgrade:false,glassShield:false,wonAll:0,cinderSeal:false,trialSealPlaced:false,trialWins:0,thornwellMet:false,brambleQuest:0,
 gold:100,potions:1,treasuryTaken:new Set(),dragon:{hp:5,maxHp:5},elixirs:0,bombs:0,dust:0,bells:0,marks:0,breaths:0,stones:0,salts:0,boarMeat:0,hareMeat:2,deerMeat:3,dragonFish:0,fishingPole:false,trial:null,activeSaveSlot:1,
...
assert.equal(saved.deerMeat,3);c.deerMeat=0;assert.equal(saved.hareMeat,2);c.hareMeat=0;assert.equal(saved.passageLayoutVersion,1);assert(saved.templeDefeated['passage3:0']);assert(Object.keys(saved.templeDefeated).some(k=>k.includes(':spikes:')));
for(const k of Object.keys(c.bossGone))delete c.bossGone[k];assert(run('loadGame(1)'));assert.deepEqual(c.bossGone,saved.templeDefeated);assert.equal(c.hareMeat,2,'hare meat survives reload');assert.equal(c.deerMeat,3,'deer meat survives reload');
for(const id of ['passage','passage2','passage3']){
 ...
 assert.equal(c.deerMeat,0,'older saves default to no deer meat');assert.equal(c.hareMeat,0,'older saves default to no hare meat');assert.deepEqual([c.P.x,c.P.y],Array.from(W.maps[id].spawn));assert.equal(Object.keys(c.bossGone).length,0,'save slots do not share passage defeats');
}
