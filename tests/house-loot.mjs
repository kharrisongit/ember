...
for(const c of placements){
 ...
 const before=reveals;assert.equal(vm.runInContext('tryHouseLootChest()',ctx),true);total+=c.gold;assert.equal(reveals,before,'reveal waits for opening animation');
 ...
 clock+=60;vm.runInContext('stepLootChestOpening()',ctx);assert.equal(reveals,before+1);
 assert.equal(vm.runInContext('tryHouseLootChest()',ctx),true);assert.equal(ctx.gold,total);
}
assert.equal(saves,79);assert.equal(ctx.actor.spr,'temple71_chest');assert.equal(ctx.potions,potions);assert.equal(ctx.boarMeat,meat);assert.equal(ctx.dragonFish,fish);
saved=JSON.parse(JSON.stringify(vm.runInContext('captureSave()',ctx)));
assert.equal(saved.houseLootTaken.length,79);assert.equal(saved.potions,potions);
...
console.log('PASS: 79 chests, 53 homes, idempotent setup, six animation frames, one-time rewards, actual save/load, slot isolation, legacy saves and moved chest interaction.');
