...
const layouts=JSON.parse(read('assets/interiors/millwood/layouts.json'));
const outside=JSON.stringify(Object.fromEntries(Object.entries(world.maps).filter(([id])=>!layouts[id])));
const ctx=vm.createContext({W:world,window:{},fetch:async()=>({ok:true,json:async()=>layouts}),Image:class{async decode(){}},document:{createElement:()=>({getContext:()=>({drawImage(){}})})},npcs:[],actorLayouts:{},SPR:{},NAMES:[],TS:16,rebuildSolid(){},mapDirty:false});
